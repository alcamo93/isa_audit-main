<script>
activeMenu(8, 'Requerimientos de auditoría'); 
...
const requirementsTable = $('#requirementsTable').DataTable({
    ...
    ajax: { 
        ...
        data: (data) => {
            ...
            data.IdCondition = document.querySelector('#filterIdCondition').value,
            ...
            data.IdRequirementType = document.querySelector('#filterIdRequirementType').value,
            // only for special requiremnets
            data.filterIdCustomer = null,
            data.filterIdCorporate = null
        }
    }, 
    columns: [
        ...
        },
        {
            render: (data, type, row) => {
                ...
                let btnEdit = (MODIFY) ? `<a class="btn btn-warning btn-link btn-xs" data-toggle="tooltip" title="Editar" 
                                    onclick="openRequirementModel(${data})">
...
function setDataInForm(isNormal) {
    return new Promise((resolve, reject) => {
        ...
        if (!isNormal) {
            document.querySelector('#IdMatter').value =  '{{ $parameters->matter_id }}'; // remover al quitar columnas
            document.querySelector('#IdAspect').value = '{{ $parameters->aspect_id }}'; // remover al quitar columnas
        }
        ...
        document.querySelector('#IdCondition').value = requirements.current.id_condition;
        document.querySelector('#IdRequirementType').value = requirements.current.id_requirement_type;
        setAplicationType( requirements.current.id_requirement_type, '#IdAplicationType', '#IdState', '#IdCity');
        if (requirements.current.id_requirement_type == 5 || requirements.current.id_requirement_type == 17) {
            setStates(requirements.current.id_application_type, '#IdState', '#IdCity')
        }
        document.querySelector('#IdAplicationType').value = requirements.current.id_application_type;
        document.querySelector('#IdState').value = requirements.current.id_state;
        setCities(requirements.current.id_state, '#IdCity')
        setTimeout(() => {
            document.querySelector('#IdCity').value = requirements.current.id_city;
            resolve(true);
        }, 1500);
    });
}
...
function getDataInForm(){
    return {
        ...
        IdRequirementType: document.querySelector('#IdRequirementType').value,
