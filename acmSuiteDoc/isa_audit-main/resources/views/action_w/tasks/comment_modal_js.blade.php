<script>
function addCommnet(title, idTask, actionInSection) {
    currentTask.idTask = idTask;
    document.querySelector('#commentsTitle').innerHTML = `<b>Comentarios sobre tarea: ${title}</b>`;
    $('.loading').removeClass('d-none');
    getCommentsServices()
    .then(data => (buildCommentList(data, actionInSection)) )
    .then(data => {
...
function buildCommentList(data, actionInSection) {
    return new Promise((resolve, reject) => {
        $('#commentsBody').find('li').remove();
        let list = '';
        if (data.length > 0) {
            const stage = (currentAP.section == 'expired') ? 2 : 1;
            data.forEach(c => {
...
function editComment(idComment, comment){
    cleanForm('#formComment');
    currentTask.idComment = idComment;
