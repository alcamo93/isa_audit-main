<template>
  <fragment>
    <b-button v-if="!enableBtnAddAudit"
      variant="success"
      block pill
      @click="showModal"
    >
...
import { storeAuditRegister } from '../../../../services/AuditRegisterService'
import { storeObligationRegister } from '../../../../services/ObligationRegisterService'
...
export default {
  ...
  computed: {
    ...
    sectionTitle() {
      return this.activateAudit ? 'Auditoría' : 'Permisos Críticos'
    },
    titleModal() {
      const sectionName = this.activateAudit ? 'Auditoría' : 'Permisos Críticos'
      return `Activar ${sectionName}` 
    },
    initDateFormat() {
  ...
  methods: {
    ...
    async setRecord() {
      try {
        ...
        const switchService = this.activateAudit ? storeAuditRegister : storeObligationRegister
        const { data } = await switchService(this.idAuditProcess, this.idAplicabilityRegister, this.form)
        this.showLoadingMixin()
        this.dialog = false
