...
<script>
import { getAction, updateActionExpired } from '../../../../services/actionPlanService'
...
export default {
  ...
  computed: {
    titleModal() {
      return `Causas de Vencimiento de Requerimiento`
    },
    titleTooltip() {
      const text = this.keyStatus == 'EXPIRED_AP' ? 'Establecer' : 'Mostrar'
      return `${text} causas de vencimiento`
    },
    validateExpired() {
      return this.keyStatus == 'EXPIRED_AP'
    },
    showExpired() {
      return this.keyStatus == 'EXPIRED_AP' || this.hasHistorialCasuse
    }
  ...
  methods: {
    ...
    async setExpired() {
      try {
        ...
        if (!isValid) return
        this.showLoadingMixin()
        const { data } = await updateActionExpired(this.idAuditProcess, this.idAplicabilityRegister, this.origin, this.idSectionRegister, this.idActionRegister, this.idActionPlan, this.form)
        const response = data
        this.dialog = false
        this.showLoadingMixin()
    ...
    async getActionPlan() {
      try {
        this.showLoadingMixin()
        const { data } = await getAction(this.idAuditProcess, this.idAplicabilityRegister, this.origin, this.idSectionRegister, this.idActionRegister, this.idActionPlan, this.form)
