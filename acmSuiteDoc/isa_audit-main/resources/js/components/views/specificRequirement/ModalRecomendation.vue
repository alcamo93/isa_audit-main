...
import { getReqRecomendations, deleteReqRecomendation, setReqRecomendation, updateReqRecomendation } from '../../../services/specificRecomendationService'
  ...
  methods: {
    ...
    async getReqRecomendations() {
      try {
        ...
        const { data } = await getReqRecomendations(this.idRequirement, params, this.filters)
    ...
    async pushRecomendation() {
      try {
        ...
        if (!isValid) return
        this.showLoadingMixin()
        const typeFunction = this.id_recomendation == null ? setReqRecomendation(this.idRequirement, this.form) : updateReqRecomendation(this.idRequirement, this.id_recomendation, this.form)
    ...
    async removeRecomendation({ id_recomendation, recomendation }) {
      try {
        ...
        if (value) {
          this.showLoadingMixin()
          const { data } = await deleteReqRecomendation(this.idRequirement, id_recomendation)
          this.responseMixin(data)
          await this.getReqRecomendations()
          this.showLoadingMixin()
        }
