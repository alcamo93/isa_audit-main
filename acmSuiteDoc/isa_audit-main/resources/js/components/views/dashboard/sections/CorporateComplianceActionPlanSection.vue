  ...
  <fragment>
    <div class="d-flex" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos en Plan de Acción para mostar aún el Cuplimiento EHS</h5>
      </b-card>
    </div>
    <filter-area v-else :title="`Plan de Acción de Cuplimiento EHS ${year} - ${customer.corp_tradename}`" :opened="true" :custom-title="true">
      ...
      <modal-condition-graph ref="modalConditionGraph" 
        ...
        :data="findings.graph" :aspects="findings.aspects" />
...
import { getDataComplianceActionCorporate } from '../../../../services/dashboardService'
  ...
  methods: {
    async getCorporateAuditActionPlan() {
      try {
        ...
        const { data } = await getDataComplianceActionCorporate(this.idAuditProcess, this.idAplicabilityRegister, this.idAuditRegister)
    ...
    showRecordsRisk({ id_risk_category, risk_category, interpretation }) {
      this.modal_risk.id_action_register = this.customer.id_action_register
