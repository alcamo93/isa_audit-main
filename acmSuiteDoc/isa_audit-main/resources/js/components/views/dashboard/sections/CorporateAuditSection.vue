  ...
  <fragment>
    ...
    <div class="d-flex" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos para mostrar aún la Auditoría</h5>
      </b-card>
    </div>
    <filter-area v-else :title="`Auditoría ${year} - ${customer.corp_tradename}`" :opened="true" :custom-title="true">
      <records-audit ref="modalRecordsAudit" 
      ...
      <template v-slot:action>
        <corporate-audit-report-button class="mr-2"
          :id-audit-process="idAuditProcess"
          :id-aplicability-register="idAplicabilityRegister"
          :id-audit-register="idAuditRegister"
...
import RecordsAudit from '../modals/RecordsAudit'
import CorporateAuditReportButton from '../reports/CorporateAuditReportButton'
import ImageItem from '../../components/customers/ImageItem'
import { getDataAuditCorporate } from '../../../../services/dashboardService'
...
export default {
  ...
  methods: {
    async getDataAuditCorporate() {
      try {
        this.showLoadingMixin()
        const { data } = await getDataAuditCorporate(this.idAuditProcess, this.idAplicabilityRegister, this.idAuditRegister)
        const { with_data, year, customer, compliance, matters, last_years } = data.data
        this.year = year
    ...
    showRecords({ aspect_name, id_aspect }) {
      this.modal_records.id_audit_processes = this.idAuditProcess
      this.modal_records.id_aplicability_register = this.idAplicabilityRegister
      this.modal_records.id_audit_register = this.idAuditRegister
      this.modal_records.id_aspect = id_aspect
      this.modal_records.aspect_name = aspect_name
      this.$refs.modalRecordsAudit.showModal()
    }
  }
}
</script>
