  ...
  <fragment>
    <div class="d-flex" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos para mostrar aún el Plan de Acción de Permisos Criticos</h5>
      </b-card>
    </div>
    <filter-area v-else :title="`plan de acción permisos críticos ${year} - ${customer.corp_tradename}`" :opened="true" :custom-title="true">
      ...
      <records-action-plan ref="modalRecordsActionPlan" 
        ...
        section-name="obligation"
        :id-section-register="customer.id_obligation_register"
      ...
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        ...
        <b-card class="flex-fill text-center m-1">
          ...
          <industrial-plant-grouped :series="monthly.series" :colors="monthly.colors" :text="'Requerimientos'"
      ...
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        <b-card class="flex-fill text-center m-1">
          <h5 class="global-dashboard-title">
            número de incumplimietos criticos
          </h5>
      ...
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        <b-card class="flex-fill text-center m-1" v-for="item in matters.data" :key="item.id">
          <h5 class="global-dashboard-title text-break">
            estatus de requerimientos críticos
          </h5>
          <h5 class="global-dashboard-title text-break">
            {{ item.matter }}
          </h5>
...
import { getCorporateObligationActionPlan } from '../../../../services/dashboardService'
...
export default {
  ...
  mounted() {
    this.getCorporateObligationActionPlan()
  },
  data() {
    return {
      year: '',
  ...
  methods: {
    async getCorporateObligationActionPlan() {
      try {
        this.showLoadingMixin()
        const { data } = await getCorporateObligationActionPlan(this.idAuditProcess, this.idAplicabilityRegister)
        const { with_data, year, monthly, project, compliance, counts, risk } = data.data
        this.year = year
    ...
    setCustomer({ corp_tradename, id_action_register, obligation_register_id}) {
      this.customer.corp_tradename = corp_tradename
      this.customer.id_action_register = id_action_register
      this.customer.id_obligation_register = obligation_register_id
    },
    ...
    setCritical(counts) {
      const aspects = counts.map(matter => matter.aspects).flat().map(aspect => {
        return { y: aspect.total_critical, x: aspect.aspect, fillColor: aspect.color }
      })
    ...
    showRecords({ id_matter, id_status, matter_name, status_name }) {
      this.modal.id_action_register = this.customer.id_action_register
    ...
    showGraph({ serie }) {
      const { id, name } = serie
      this.modal_graph.id_action_register = this.customer.id_action_register
