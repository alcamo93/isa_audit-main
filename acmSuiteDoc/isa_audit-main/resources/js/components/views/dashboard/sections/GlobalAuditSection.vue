  ...
  <fragment>
    ...
    <div class="d-flex" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos de auditoría</h5>
      </b-card>
    </div>
    <filter-area v-else
      :title="`auditorías ${year}`"
      :opened="true"
      :custom-title="true"
    >
      <template v-slot:action>
        <global-audit-report-button class="mr-2"
      ...
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        ...
        <b-card class="flex-fill text-center m-1">
          <h5 class="global-dashboard-title">
            resultados de auditorías - plantas {{ year }}
          </h5>
...
import GlobalAuditReportButton from '../reports/GlobalAuditReportButton'
import ImageItem from '../../components/customers/ImageItem'
import { getCustomerGlobalAudit } from '../../../../services/dashboardService'
...
export default {
  ...
  methods: {
    async getCustomerGlobalAudit() {
      try {
        this.showLoadingMixin()
        const { data } = await getCustomerGlobalAudit(this.idCustomer)
    ...
    goToCorporate({id_audit_processes, id_aplicability_register, id_audit_register}) {
      ...
      const url = `${host}/v2/dashboard/project/${id_audit_processes}/applicability/${id_aplicability_register}/audit/${id_audit_register}/view`
