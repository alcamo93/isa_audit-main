  ...
  <fragment>
    ...
    <div class="d-flex" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos históricos en Auditoría y Plan de Acción para mostrar aún Cumplimiento EHS</h5>
      </b-card>
    </div>
    <filter-area v-else title="histórico - cumplimiento EHS" :opened="true" :custom-title="true">
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        <b-card class="flex-fill text-center m-1 w-25">
          <h5 class="global-dashboard-title">
            cumplimiento EHS global - registro anual
          </h5>
        ...
        <b-card class="flex-fill text-center m-1">
          <h5 class="global-dashboard-title">
            cumplimiento EHS por planta - registro anual
          </h5>
...
import { getCustomerGlobalComplianceHistorical } from '../../../../services/dashboardService'
...
export default {
  ...
  mounted() {
    this.getCustomerGlobalComplianceHistorical()
  },
  data() {
    return {
      with_data: false,
  ...
  methods: {
    async getCustomerGlobalComplianceHistorical() {
      try {
        this.showLoadingMixin()
        const { data } = await getCustomerGlobalComplianceHistorical(this.idCustomer)
        ...
        this.setProjects(global_projects, last_years.data)
        this.showLoadingMixin()
      } catch (error) {
        this.showLoadingMixin()
