  ...
  <fragment>
    ...
    <div class="d-flex" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos históricos para mostrar aún Permisos Críticos</h5>
      </b-card>
    </div>
    <filter-area v-else title="histórico - Permisos Críticos" :opened="true" :custom-title="true">
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        <b-card class="flex-fill text-center m-1 w-25">
          ...
          <last-three-years :customer-id="idCustomer" :chart-id="`last-three-years-customer-id-ob-${idCustomer}`"
        ...
        <b-card class="flex-fill text-center m-1">
          ...
          <industrial-plant-grouped :customer-id="idCustomer"
            :chart-id="`industrial-plant-grouped-last-three-years-customer-id-ob-${idCustomer}`"
...
import { getCustomerGlobalObligationHistorical } from '../../../../services/dashboardService'
...
export default {
  ...
  mounted() {
    this.getCustomerGlobalObligationHistorical()
  },
  data() {
    return {
      with_data: false,
  ...
  methods: {
    async getCustomerGlobalObligationHistorical() {
      try {
        this.showLoadingMixin()
        const { data } = await getCustomerGlobalObligationHistorical(this.idCustomer)
