<template>
  <fragment>
    ...
    <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap" v-if="!with_data">
      <b-card class="flex-fill text-center">
        <h5>No se cuenta con datos para Permisos Críticos</h5>
      </b-card>
    </div>
    <filter-area v-else :title="`Cumplimiento Global Permisos Críticos ${year}`" :opened="true" :custom-title="true">
      <template v-slot:action>
        <global-obligation-report-button class="mr-2"
      ...
      <div class="d-flex flex-wrap flex-sm-wrap flex-md-nowrap">
        ...
        <b-card class="flex-fill text-center m-1">
          <h5 class="global-dashboard-title">
            permisos críticos - plantas {{ year }}
          </h5>
...
import GlobalObligationReportButton from '../reports/GlobalObligationReportButton'
import ImageItem from '../../components/customers/ImageItem'
import { getCustomerGlobalObligation } from '../../../../services/dashboardService'
...
export default {
  ...
  mounted() {
    this.getCustomerGlobalObligation()
  },
  data() {
    return {
      year: '',
  ...
  methods: {
    async getCustomerGlobalObligation() {
      try {
        this.showLoadingMixin()
        const { data } = await getCustomerGlobalObligation(this.idCustomer)
        const { year, customer, global_matters, global_projects,
          global_compliance, global_historical_month, with_data } = data.data
        this.year = year
        ...
        this.setMonth(global_historical_month)
        this.showLoadingMixin()
      } catch (error) {
        this.showLoadingMixin()
    ...
    setAllProcess(global_projects) {
      ...
      this.process.main = global_projects.sort((a, b) => a.obligation_register_id - b.obligation_register_id)
      this.process.colors = global_matters.map(item => item.color)
      this.process.series = global_matters.map(item => {
        return {
          id: item.obligation_register_id,
          name: item.matter,
    ...
    setMonth(global_historical_month) {
      const data = global_historical_month.map(({ key, total }) => {
    ...
    goToCorporate({id_audit_processes, id_aplicability_register, id_obligation_register}) {
      const host = window.location.origin
      const url = `${host}/v2/dashboard/project/${id_audit_processes}/applicability/${id_aplicability_register}/obligation/${id_obligation_register}/view`
