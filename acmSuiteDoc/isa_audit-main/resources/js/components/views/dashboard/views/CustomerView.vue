<template>
  <fragment>
    ...
    <b-container fluid>
      ...
      >
        <!-- item customer -->
        <div class="flex-fill flex-sm-fill flex-md-grow-0 flex-lg-grow-0 m-0 flex-xl-grow-0 m-1" v-if="customer != null"
        ...
        <!-- items corporates -->
        <div class="flex-fill flex-sm-fill flex-md-grow-0 flex-lg-grow-0 m-0 flex-xl-grow-0 m-1 m-1"
          v-for="item in items"
...
import { getCorporatesByCustomer } from '../../../../services/dashboardService'
...
export default {
  ...
  computed: {
    customer() {
      ...
      const { customer } = this.items[0]
      return customer
    }
  },
  methods: {
    async getCorporatesByCustomer() {
      try {
        this.showLoadingMixin()
        const { data } = await getCorporatesByCustomer({}, this.filters)
    ...
    goToCorporate({id_audit_processes, aplicability_register}) {
      const idAplicabilityRegister = aplicability_register.id_aplicability_register
      const host = window.location.origin
      const url = `${host}/v2/dashboard/project/${id_audit_processes}/applicability/${idAplicabilityRegister}/all/view`
