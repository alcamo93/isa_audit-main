...
import { getRelationArticles, setRelationLegal } from '../../../../../../services/questionLegalsService'
...
export default {
  ...
  computed: {
    ...
    titleTooltip() {
      if (this.record == null) return ''
      return `Selección de fundamentos legales`
    },
    headerTable() {
      return [
        {
  ...
  methods: {
    ...
    async getArticles() {
      try {
        ...
        const { data } = await getRelationArticles(this.idForm, this.record.id_question, params, this.filters)
    ...
    hasRelation(type, {questions}) {
      if (type == 'message') {
        return Boolean(questions.length) ? 'Remover relación de articulo con pregunta' : 'Relacionar artículo con pregunta'
      }
      if (type == 'icon') {
        return Boolean(questions.length) ? 'patch-check-fill' : 'patch-minus-fill'
      }
      if (type == 'color') {
        return Boolean(questions.length) ? 'success' : 'warning'
      }
    },
    async setLegal({id_legal_basis}) {
      try {
        this.showLoadingMixin()
        const { data } = await setRelationLegal(this.idForm, this.record.id_question, id_legal_basis)
        await this.getArticles()
        this.showLoadingMixin()
      } catch (error) {
        this.showLoadingMixin()
