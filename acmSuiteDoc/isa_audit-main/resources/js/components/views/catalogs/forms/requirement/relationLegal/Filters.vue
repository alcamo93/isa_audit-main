...
<script>
import { getRelationGuideline } from '../../../../../../services/requirementLegalsService'
...
export default {
  ...
  computed: {
    ...
    stateName() {
      if (this.record.id_requirement_type == 11) {
        return 'Consulte estado de la Planta'
      } 
    ...
    cityName() {
      if (this.record.id_requirement_type == 11) {
        return 'Consulte ciudad de la Planta'
      } 
  ...
  methods: {
    ...
    async getGuidelines() {
      try {
        ...
        const { data } = await getRelationGuideline(this.idForm, this.idRequirement, {}, filters)
