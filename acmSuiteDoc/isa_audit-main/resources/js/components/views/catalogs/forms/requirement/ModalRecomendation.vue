...
import { getReqRecomendations, deleteReqRecomendation, setReqRecomendation, updateReqRecomendation } from '../../../../../services/recomendationService'
...
export default {
  ...
  methods: {
    async showModal() {
      ...
      this.reset()
      await this.getReqRecomendations()
      this.showLoadingMixin()
      this.dialog = true
    },
    async getReqRecomendations() {
      try {
        this.showLoadingMixin()
        const params = {
        ...
        const { data } = await getReqRecomendations(this.idForm, this.idRequirement, params, this.filters)
    ...
    async pushRecomendation() {
      try {
        ...
        if (!isValid) return
        this.showLoadingMixin()
        const typeFunction = this.id_recomendation == null ? setReqRecomendation(this.idForm, this.idRequirement, this.form) : updateReqRecomendation(this.idForm, this.idRequirement, this.id_recomendation, this.form)
    ...
    async removeRecomendation({ id_recomendation, recomendation }) {
      try {
        ...
        if (value) {
          this.showLoadingMixin()
          const { data } = await deleteReqRecomendation(this.idForm, this.idRequirement, id_recomendation)
          this.responseMixin(data)
          await this.getReqRecomendations()
          this.showLoadingMixin()
        }
