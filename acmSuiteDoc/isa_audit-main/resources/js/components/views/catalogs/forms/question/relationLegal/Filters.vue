...
<script>
import { getRelationGuideline } from '../../../../../../services/questionLegalsService'
...
export default {
  ...
  computed: {
    idQuestion() {
      return this.record.id_question
    },
    questionType() {
      const string = this.record.type.question_type
      return string
    },
    questionString() {
      const string = this.record.question
      return string
    },
    stateName() {
  ...
  methods: {
    ...
    async getGuidelines() {
      try {
        ...
        const { data } = await getRelationGuideline(this.idForm, this.idQuestion, {}, filters)
