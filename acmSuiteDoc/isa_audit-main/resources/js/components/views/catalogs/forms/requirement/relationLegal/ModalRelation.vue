...
import ModalLegalQuote from '../../../guideline/legal_basi/ModalLegalQuote.vue'
import Filters from './Filters.vue'
import { getRelationArticles, setRelationLegal } from '../../../../../../services/requirementLegalsService'
...
export default {
  ...
  computed: {
    titleModal() {
      ...
      return `Fundamentos legales para: ${this.record.no_requirement}`
    },
    titleTooltip() {
      if (this.record == null) return ''
      return `Selección de fundamentos legales para: ${this.record.no_requirement}`
    },
    headerTable() {
      return [
        {
  ...
  methods: {
    ...
    async getArticles() {
      try {
        ...
        const { data } = await getRelationArticles(this.idForm, this.record.id_requirement, params, this.filters)
    ...
    hasRelation(type, {requirements}) {
      if (type == 'message') {
        return Boolean(requirements.length) ? 'Remover relación de articulo con requerimiento' : 'Relacionar artículo con requerimiento'
      }
      if (type == 'icon') {
        return Boolean(requirements.length) ? 'patch-check-fill' : 'patch-minus-fill'
      }
      if (type == 'color') {
        return Boolean(requirements.length) ? 'success' : 'warning'
      }
    },
    async setLegal({id_legal_basis}) {
      try {
        this.showLoadingMixin()
        const { data } = await setRelationLegal(this.idForm, this.record.id_requirement, id_legal_basis)
        await this.getArticles()
        this.showLoadingMixin()
      } catch (error) {
        this.showLoadingMixin()
