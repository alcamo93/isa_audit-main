...
export default {
  ...
  computed: {
    ...
    titleModal() {
      ...
      return `Ayuda para el auditor en ${this.getNameRecord}`
    },
    titleTooltip() {
      if (this.record == null) return ''
      return `Ver ayuda para el auditor en ${this.getNameRecord}`
    },
  },
  methods: {
    async showModal() {
    ...
    async getRecord() {
      try {
        ...
        this.text = this.isRequirement ? data.data.help_requirement : data.data.help_subrequirement
        this.showLoadingMixin()
      } catch (error) {
        this.showLoadingMixin()
