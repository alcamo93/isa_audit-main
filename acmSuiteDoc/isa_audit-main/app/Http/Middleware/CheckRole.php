...
class CheckRole
{
    ...
    public function handle($request, Closure $next){
        // Get Module by path
        $pathCurrent = $currentPath = Route::getFacadeRoot()->current()->uri();
        $splitString = explode('/', $pathCurrent);
        $path = ($splitString[0] == 'corporates') ? 'customers' : $splitString[0];
        $module = ModulesModel::getModulePath($path);
        if(is_object($module) || !isset($module['id_module'])){
        ...
        foreach($activeModules as $index => $mod) {
            ...
            $activeModules[$index]['active'] = $request->is($mod['path']);
        ...
        $permission = ProfilesPermissionModel::getPermission($module['id_module'], $userInfo[0]['id_profile']);
        //dd($permission);
