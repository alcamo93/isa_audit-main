...
class ReportFindingSheet implements FromView, WithTitle, WithStyles, ShouldAutoSize, WithColumnWidths
{
	use Exportable;
	...
	{
		return view('exports.audit.report_finding', [
			'data' => $this->data,
			'risk' => $this->risk
		]);
	}
	...
	{
		$range = range('A', 'I');
		$columns = collect($range)->map(fn($item) => [$item => 30])->collapse()->toArray();
		$columns['A'] = 5;
		return $columns;
	}
	...
	public function styles(Worksheet $sheet)
	{
		...
		$counterMainRow = $aspects->sum( fn($item) => sizeof($item['findings']) );
		$counterSubRow = $aspects->pluck('findings')->collapse()->sum( fn($item) => sizeof($item['child_findings']) );
