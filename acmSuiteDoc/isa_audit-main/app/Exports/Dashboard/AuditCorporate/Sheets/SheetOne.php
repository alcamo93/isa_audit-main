...
class SheetOne implements FromView, WithTitle, WithDrawings, WithStyles
{
	...
	{
		return view('exports.dashboard.auditCorporate.sheets.sheetOne', [
			'data' => $this->data
		]);
	}
    ...
    for ($i=0; $i < $rowRepit ; $i++) { 
      ...
      if ($i == 2) {
        $sheet->getStyle("B{$currentRow}:V$lastRow")->applyFromArray(Style::STYLE_BORDER_MAIN)->applyFromArray(Style::STYLE_CARD_HEADERS);
        break;
      }
