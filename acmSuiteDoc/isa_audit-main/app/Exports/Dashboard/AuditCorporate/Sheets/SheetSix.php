...
class SheetSix implements FromView, WithTitle, WithDrawings, WithStyles, WithColumnWidths
{
	...
	{
		return view('exports.dashboard.auditCorporate.sheets.sheetSix', [
			'data' => $this->data
		]);
	}
    ...
    // header
		...
		$sheet->getStyle('B2:H2')->applyFromArray(Style::STYLE_SUBTITLE_DOCUMENT);
    // table
		$sheet->getStyle("B3:H3")->applyFromArray(Style::STYLE_HEADER_TABLE);
		for ($i=0; $i <sizeof($this->data['records']) ; $i++) { 
			$row = 4 + $i;
			$sheet->getStyle("B{$row}:H{$row}")->applyFromArray(Style::STYLE_BODY_TABLE);
		}
	}
}
