...
class SheetFive implements FromView, WithTitle, WithDrawings, WithStyles, WithColumnWidths
{
	...
	{
		return view('exports.dashboard.obligationGlobal.sheets.sheetFive', [
			'data' => $this->data
		]);
	}
    ...
    $sheet->getRowDimension(2)->setRowHeight(70);
		...
		$sheet->getStyle('B2:H3')->applyFromArray(Style::STYLE_TITLE_DOCUMENT)->applyFromArray(Style::STYLE_BORDER_MAIN);
		// header table
		$sheet->getStyle('B5:H5')->applyFromArray(Style::STYLE_HEADER_TABLE);
		// historical
		$rowInit = 6;
		$historical = $this->data['projects'];
		$rowLast = ( $rowInit + sizeof($historical) ) - 1;
		for ($i=0; $i < sizeof($historical); $i++) { 
			$row = $i + $rowInit;
			$sheet->getStyle("B{$row}:H{$row}")->applyFromArray(Style::STYLE_HEADER_TABLE_NO_BORDER);
		}
		$sheet->getStyle("B{$rowLast}:H{$rowLast}")->applyFromArray(Style::STYLE_HEADER_TABLE_NO_BORDER_FOTTER);
		$sheet->getStyle("B{$rowInit}:H{$rowLast}")->applyFromArray(Style::STYLE_BORDER_MAIN);
	}
}
