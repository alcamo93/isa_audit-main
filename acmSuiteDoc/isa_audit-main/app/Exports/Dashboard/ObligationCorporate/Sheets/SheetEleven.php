...
class SheetEleven implements FromView, WithTitle, WithDrawings, WithStyles, WithColumnWidths
{
	use Exportable;
	...
	{
		return view('exports.dashboard.obligationCorporate.sheets.sheetEleven', [
			'data' => $this->data
		]);
	}
    ...
    // header
		...
		$sheet->getStyle('B2:O2')->applyFromArray(Style::STYLE_SUBTITLE_DOCUMENT);
    // table
		$sheet->getStyle("B3:O3")->applyFromArray(Style::STYLE_HEADER_TABLE);
		for ($i=0; $i <sizeof($this->data['records']) ; $i++) { 
			$row = 4 + $i;
			$sheet->getStyle("B{$row}:O{$row}")->applyFromArray(Style::STYLE_BODY_TABLE);
		}
	}
}
