...
class SheetOne implements FromView, WithTitle, WithDrawings, WithStyles
{
	...
	{
		return view('exports.dashboard.complianceGlobal.sheets.sheetOne', [
			'data' => $this->data
		]);
	}
    ...
    foreach ($matters as $index => $matter) {
      ...
      if ( $imageMatter && !is_null($imageMatter) ) {
        ...
        $place->setHeight(Variable::IMAGE_SIZE_LOGO_CUSTOMER);
        $place->setCoordinates("{$currentColumn}5");
        $place->setOffsetY(5);
        $place->setOffsetX(10);
        array_push($draws, $place);
      }
    ...
    $sheet->getRowDimension(2)->setRowHeight(60);
		$sheet->getStyle('B2:V2')->applyFromArray(Style::STYLE_TITLE_DOCUMENT)->applyFromArray(Style::STYLE_BORDER_MAIN);
    // place and logo
    $sheet->getRowDimension(4)->setRowHeight(30); // title card
    $sheet->getRowDimension(5)->setRowHeight(85); // image
    $sheet->getRowDimension(6)->setRowHeight(25); // address or porcentage
    $sheet->getRowDimension(4)->setRowHeight($this->calculateRowHeight($sheet, 'B4:E4'));
    $sheet->getRowDimension(6)->setRowHeight($this->calculateRowHeight($sheet, 'B6:E6'));
    // styles border place and logo
    $sheet->getStyle('B4:E6')->applyFromArray(Style::STYLE_BORDER_MAIN)->applyFromArray(Style::STYLE_CARD_CUSTOMER);
    $sheet->getStyle('G4:V6')->applyFromArray(Style::STYLE_BORDER_MAIN)->applyFromArray(Style::STYLE_CARD_HEADERS);
    // Borders charts
    $sheet->getStyle('B8:I24')->applyFromArray(Style::STYLE_BORDER_MAIN);
    $sheet->getStyle('K8:V24')->applyFromArray(Style::STYLE_BORDER_MAIN);
    $sheet->getStyle('B26:V42')->applyFromArray(Style::STYLE_BORDER_MAIN);
    // historical header
    $sheet->getStyle('B44:V45')->applyFromArray(Style::STYLE_TITLE_DOCUMENT)->applyFromArray(Style::STYLE_BORDER_MAIN);
    $sheet->getStyle('B47:I63')->applyFromArray(Style::STYLE_BORDER_MAIN);
    $sheet->getStyle('K47:V63')->applyFromArray(Style::STYLE_BORDER_MAIN);
	}
}
