...
class DashabordObligationCustomer
{
  ...
  {
    $completeProjects = ProcessAudit::getProcessKpiWithSpecificSectionToEvaluate('obligation', $this->currentYear, $this->idCustomer)->get();
    
    $corporates = $completeProjects->pluck('id_corporate')->unique();
    
    $projects = $corporates->map(function($idCorporate) use ($completeProjects) {
      $collection = $completeProjects->where('id_corporate', $idCorporate)->pluck('aplicability_register.obligation_register');
      $obligationRegister = $collection->where( 'id', $collection->max('id') )->first();
      return $completeProjects->firstWhere('aplicability_register.obligation_register.id', $obligationRegister->id); 
    });
    
    $instances = collect([]);
    
    foreach ($projects as $project) {
      $instance = new DashabordObligation($project->aplicability_register->obligation_register->id);
      $instances->push($instance);
    }
    
    return $instances->sortBy('obligation_register_id')->values();
  }
