...
class BuildDataReportCorporateObligation 
{
  protected $global;
  protected $action;
   ...
   */
  public function __construct($idAuditProcess, $idAplicabilityRegister)
  {
    $dashboard = new DashboardObligation();
    $this->global = $dashboard->getDataByCorporateObligation($idAuditProcess, $idAplicabilityRegister);
    $this->action = $dashboard->getDataObligationActionCorporate($idAuditProcess, $idAplicabilityRegister);
    $this->recordsTasks = $dashboard->getDataActionPlanRecords($idAuditProcess, $idAplicabilityRegister);
    $this->recordsTasksGrpuped = $dashboard->getDataActionPlanRecords($idAuditProcess, $idAplicabilityRegister, true);
    $this->historical = [ 
      'historical_last_years' => $this->global['historical_last_years'], 
      'historical_month' => $this->global['historical_month'],
      'historical_project' => $this->global['historical_project']
    ];
  ...
  private function generateCharts()
  {
    try {
      ...
      // Bar charts
      $info['obligations']['matters'] = collect($this->global['customer']['matters'])->map(function($matter) {
      ...
      $chart = $this->charts->getShortUrl($config);
      $info['obligations']['compliance'] = $chart['chart'];
      ...
      $config = null;
      $config['headers'] = [ 'title' => "Históricos Permisos Críticos", 'subtitle' => $this->corporateName ];
      $config['type'] = 'bar-vertical';
      $config['data'] = $chartStructure;
      ...
      $chart = $this->charts->getShortUrl($config);
      $info['obligations']['compliance_per_year'] = $chart['chart'];
      
      // historical chart last year per corporate
      $chartStructure = $this->structure->areaChart($this->historical['historical_month']);
      $config = null;
      $config['headers'] = [ 'title' => "Permisos Críticos - Cumplimiento Global Mensual {$this->global['year']}" ];
      ...
      $chart = $this->charts->getShortUrl($config);
      $info['obligations']['compliance_monthly'] = $chart['chart'];
  ...
  private function getDataSheetOne()
  {
    try {
      ...
      $info['header'] = "REPORTE PERMISOS CRÍTICOS {$year}";
      ...
      // matters
      $info['charts']['matters'] = $this->charts['obligations']['matters'];
      // charts
      $info['charts']['compliance'] = $this->charts['obligations']['compliance'];
      $info['charts']['compliance_per_year'] = $this->charts['obligations']['compliance_per_year'];
      $info['charts']['compliance_monthly'] = $this->charts['obligations']['compliance_monthly'];
  ...
  private function getDataSheetTwo()
  {
    try {
      ...
      // title 
      $info['header'] = 'REPORTE DE PERMISOS CRÍTICOS';
      ...
      // global total counts
      $globalsValues['total_counts']['title'] = 'PERMISOS CRÍTICOS VENCIDOS/SIN DOCUMENTO';
      $globalsValues['total_counts']['subtitle'] = 'Detalle de permisos críticos vencidos o sin Documento';
  ...
  private function getDataSheetThreeAndFour($sheet)
  {
    try {
      ...
      // title 
      $info['header'] = $sheet == 3 ? 'REPORTE DE PERMISOS CRÍTICOS' : 'No. PERMISOS CRÍTICOS VENCIDOS/SIN DOCUMENTO';
      $info['subheader'] = $this->corporateName;
  ...
  private function getDataSheetFive()
  {
    try {
      ...
      // title 
      $info['header'] = "REPORTE DE PERMISOS CRÍTICOS MENSUAL";
      $info['subheader'] = $this->corporateName;
  ...
  private function getDataSheetSix()
  {
    try {
      ...
      // title 
      $info['header'] = "ESTATUS PLAN DE ACCIÓN PERMISOS CRÍTICOS {$year}";
      $info['subheader'] = $this->corporateName;
      // place
      $info['place']['address'] = $this->global['customer']['address'];
  ...
  private function getDataSheetSeven()
  {
    try {
      ...
      // title 
      $info['header'] = "REPORTE DE PLAN DE ACCIÓN PERMISOS CRÍTICOS";
      $info['subheader'] = $this->corporateName;
      // headers
      $timezone = Config('enviroment.time_zone_carbon');
      $today = Carbon::now($timezone)->format('d-m-Y');
      ...
      $info['action_plan']['header_tasks'] = 'Estatus de Tareas de Plan de acción de Permisos Críticos';
      // for same table
  ...
  private function getDataSheetEight()
  {
    try {
      ...
      // title 
      $info['header'] = 'REPORTE DE PLAN DE ACCIÓN DE PERMISOS CRÍTICOS';
      $info['subheader'] = $this->corporateName;
      $info['description'] = "ASPECTOS POR MATERIA LEGAL";
      // matters
  ...
  private function getDataSheetNine()
  {
    try {
      ...
      // title 
      $info['header'] = 'ESTATUS MENSUAL DE PLAN DE ACCIÓN: PERMISOS CRÍTICOS';
      $info['subheader'] = $this->corporateName;
      // matters
      ...
      // build struture matters
      $info['matters'] = collect($this->action['historical_project']['matters']);
  ...
  private function getDataSheetTen()
  {
    try {
      ...
      // title 
      $info['header'] = 'TAREAS DE PLAN DE ACCIÓN DE PERMISOS CRÍTICOS';
      $info['subheader'] = $this->corporateName;
  ...
  private function getDataSheetEleven()
  {
    try {
      ...
      // title 
      $info['header'] = 'REPORTE DE TAREAS  DE PLAN DE ACCIÓN DE PERMISOS CRÍTICOS';
      $info['subheader'] = $this->corporateName;
      // records tasks
      ...
      $info['success'] = false;
      $info['message'] = 'Algo salio mal al generar la primero decima hoja del reporte';
      return $info;
    }
  }
}