...
class CreateRAuditAspects extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('r_audit_aspects', function (Blueprint $table) {
            $table->bigIncrements('id_audit_aspect');
            ...
            $table->double('total')->default(0)->comment('Specify percentage compliance per aspect');
            $table->unsignedBigInteger('id_audit_matter')->comment('Foreign key referring to aplicability registers table');
        ...
        Schema::table('r_audit_aspects', function($table) {
            $table->foreign('id_audit_matter')->references('id_audit_matter')->on('r_audit_matters')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('r_audit_aspects');
    }
}
