...
class CreateTEvaluateRequirement extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('t_evaluate_requirement', function (Blueprint $table) {
            $table->bigIncrements('id_evaluate_requirement');
            $table->tinyInteger('complete')->default(0)->comment('The value 1 is complete, 0 no complete');
            $table->unsignedBigInteger('id_contract_aspect')->comment('Foreign key referring to aplicability aspect');
        ...
        Schema::table('t_evaluate_requirement', function($table) {
    ...
    {
        Schema::dropIfExists('t_evaluate_requirement');
    }
}
