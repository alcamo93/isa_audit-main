...
class CreateTRequirements extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('t_requirements', function (Blueprint $table) {
            $table->bigIncrements('id_requirement')->comment('Primary key referring to requirements table');
            $table->string('no_requirement')->comment('Specify the keyword to the requirement');
            $table->text('requirement')->comment('Specify the requirement requested in the audit');
            $table->text('description')->nullable()->comment('Specify the description requirement requested in the audit');
            $table->longText('help_requirement')->nullable()->comment('Auditors help');
            $table->text('acceptance')->nullable()->comment('Acceptance criterion');
            $table->tinyInteger('has_subrequirement')->default(0)->comment('The value 1 refers to it has subrequirents, 0 to has not');
            ...
            $table->unsignedBigInteger('id_application_type')->nullable()->comment('Foreign key referring to requiriment types table');
